    ...
    useEffect(() => {
        const fetchCourseData = async () => {
            try {
                setLoading(true);
                // Fetch single course
                const courseRes = await axios.get(`${import.meta.env.VITE_API_URL}/api/Course/${id}`);
                setCourse(courseRes.data?.course);

                // Fetch reviews for course
                const reviewsRes = await axios.get(`${import.meta.env.VITE_API_URL}/api/reviews/${id}`);
